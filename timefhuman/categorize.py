...
from .constants import MEETING_WORDS
...
def categorize(tokens, now):
    ...
    """
    # Have to call extract_meeting_words before calling any of the date functions, as the tokens variable is overwritten
    tokens = list(tokens)

    meeting_words_extracted = extract_meeting_words(tokens)
    ...
    tokens = convert_time_delta_values(tokens)
    ...
    return tokens, meeting_words_extracted

# TODO: extract words that suggest meeting eg. let us meet at 4. Let us have a call tommorrow, etc


def extract_meeting_words(tokens):
    """
    Extracts words from string that are present in MEETING_WORDS constant tuple and return a list
    >>> extract_meeting_words(['let', 'us', 'meet', 'at', '4'])
    ['meet']
    """
    meeting_words_extracted = []
    for token in tokens:
        if isinstance(token, str) and token.lower() in MEETING_WORDS:
            meeting_words_extracted.append(token.lower())
    return meeting_words_extracted
...
def convert_day_of_week(tokens, now=datetime.datetime.now()):
    ...
    for i in range(7):
        ...
        for string in (day_of_week, day_of_week[:3], day_of_week[:4]):
            for index, token in enumerate(tokens):
                if isinstance(token, str) and string.lower() == token.lower():
                    new_index, tokens, weeks = extract_weeks_offset(
                        tokens, end=index)
...
def convert_relative_days_ranges(tokens, now=datetime.datetime.now()):
    ...
    for keywords, replacement in (
            ...
            (("tomorrow", "tmw"), DayToken.from_datetime(
...
def convert_time_delta_values(tokens):
    """Convert time deltas like in 10 mins or in 4 hours etc
    >>> convert_time_of_day(['today', '10', 'mins', 'huehue'])
    ['today', currentTime + 10 mins, 'huehue']
    """
    temp_tokens = [token.lower() if isinstance(
        token, str) else token for token in tokens]
    for keyword, time_token in (('mins', 'm'), ('minutes', 'm'), ('minute', 'm'), ('hours', 'h'), ('hour', 'h'), ('hrs', 'h')):
        if keyword in temp_tokens:
            index = temp_tokens.index(keyword)
            try:
                time_delta = int(temp_tokens[index - 1])
                ...
                tokens = tokens[:index - 1] + [time] + tokens[index+1:]
            except:
                continue
    return tokens
...
def convert_time_of_day(tokens):
    ...
    ['Monday', 3 pm]
    >>> convert_time_of_day(['Tue', 'evening'])
    ['Tue', 6 pm]
...
def maybe_substitute_using_month(tokens, now=datetime.datetime.now()):
    ...
    for mo, month in enumerate(MONTHS, start=1):
        ...
        if next_next_candidate == ',':
            next_next_candidate = tokens[index +
                                         3] if len(tokens) > index+3 else ''
...
def maybe_substitute_hour_minute(tokens):
    ...
    [12 pm]
    """
    def remove_dots(token): return token.replace('.', '')
    temp_tokens = clean_tokens(tokens, remove_dots)

    for time_of_day in ('am', 'pm'):
        while time_of_day in temp_tokens:
            index = temp_tokens.index(time_of_day)
            (unchanged_index, time_token) = extract_hour_minute_token(
                temp_tokens[:index], time_of_day)
            tokens = tokens[:index+unchanged_index] + \
                [time_token] + tokens[index+1:]
            temp_tokens = clean_tokens(tokens, remove_dots)
